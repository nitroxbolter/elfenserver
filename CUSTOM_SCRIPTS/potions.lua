...
local potions = {
	...
	[26736] = { -- nimble mana potion
		...
		level = 100,
		flask = 26753,
		description = "Only sorcerers, druids and paladins of level 100 or above may drink this fluid."
	},
	[26737] = { -- hazel mana potion
		...
		level = 150,
		flask = 26754,
		description = "Only paladins, druids and sorcerers of level 150 or above may drink this fluid."
	},
...
function onUse(player, item, fromPosition, target, toPosition, isHotkey)
	...
	local now = os.time()
    local cooldown = 10
	if not lastUsageTime[player:getId()] or now - lastUsageTime[player:getId()] >= cooldown then
        lastUsageTime[player:getId()] = now
    ...
    local lastUsage = lastUsageTime[player:getId()]
    local remainingTime = lastUsage + cooldown - now
    if remainingTime > 0 then
        player:say("You are in cooldown. Remaining time: " .. remainingTime .. " seconds", TALKTYPE_MONSTER_SAY)
    else
        player:say("You must wait for the cooldown before using this item again.", TALKTYPE_MONSTER_SAY)
    end
    return true
end
