...
local categories = {"herbalist", "woodcutting", "mining", "ressources", "equipments"}
...
function Crafting:craft(player, category, craftId)
  ...
  local playerSkill = player:getEffectiveSkillLevel(SKILL_CRAFTING)
  local money = player:getMoney()

  if money < craft.cost then
    player:popupFYI(
        string.format("You don\'t enought money: " .. craft.cost .. ".")
      )
    return
  end
  ...
  if playerSkill < craft.level then
    player:popupFYI(
        string.format("You don\'t have required Crafting Skill: " .. craft.level .. ", your craft skill is: " .. playerSkill ..".")
      )
    return
  end
  ...
  for i = 1, #craft.materials do
    ...
    if player:getItemCount(material.id) < material.count then
      player:popupFYI(
        string.format("You don\'t enought materials.")
      )
      return
    end
  end
  ...
  if item then
    if player:addItemEx(item) then
      ...
      player:addSkillTries(SKILL_CRAFTING, skillTries)
      Crafting:sendMoney(player)
