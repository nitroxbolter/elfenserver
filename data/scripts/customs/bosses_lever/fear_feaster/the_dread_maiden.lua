local config = {
    ...
    bossPosition = Position(1592, 514, 9), 
    bossArea = {
        fromPos = Position(1578, 505, 9), 
        toPos = Position(1609, 531, 9), 
        entrancePos = Position(1593, 524, 9), 
        exitPosition = Position(1621, 522, 9) 
    },
    ...
    createTeleportPos = Position(1593, 510, 9), 
    teleportToPosition = Position(1621, 522, 9), 
...
local theDreadAction = Action()

function theDreadAction.onUse(player, item, fromPos, target, toPos, isHotkey)
    local participants = {}
    ...
    boss:registerEvent("theDreadSystemDeath")
...
theDreadAction:aid(config.actionId)
theDreadAction:register()

local creatureEvent = CreatureEvent("theDreadSystemDeath")
