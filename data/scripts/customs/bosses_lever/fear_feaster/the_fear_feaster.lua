local config = {
    ...
    bossPosition = Position(1658, 468, 9), 
    bossArea = {
        fromPos = Position(1648, 459, 9), 
        toPos = Position(1665, 478, 9), 
        entrancePos = Position(1658, 474, 9), 
        exitPosition = Position(1681, 471, 9) 
    },
    ...
    createTeleportPos = Position(1657, 461, 9), 
    teleportToPosition = Position(1681, 471, 9), 
...
local theFeardAction = Action()

function theFeardAction.onUse(player, item, fromPos, target, toPos, isHotkey)
    local participants = {}
    ...
    boss:registerEvent("theFearSystemDeath")
...
theFeardAction:aid(config.actionId)
theFeardAction:register()

local creatureEvent = CreatureEvent("theFearSystemDeath")
