local config = {
    actionId = 42583, 
    bossName = "sir baeloc",
    bossPosition = Position(829, 420, 9), 
    bossArea = {
        fromPos = Position(817, 408, 9), 
        toPos = Position(843, 432, 9), 
        entrancePos = Position(829, 429, 9), 
        exitPosition = Position(830, 389, 9) 
    },
    ...
    createTeleportPos = Position(821, 420, 9), 
    teleportToPosition = Position(830, 389, 9), 
...
local sirNictrosAction = Action()

function sirNictrosAction.onUse(player, item, fromPos, target, toPos, isHotkey)
    local participants = {}
    ...
    boss:registerEvent("sirNictrosSystemDeath")
...
sirNictrosAction:aid(config.actionId)
sirNictrosAction:register()

local creatureEvent = CreatureEvent("sirNictrosSystemDeath")
