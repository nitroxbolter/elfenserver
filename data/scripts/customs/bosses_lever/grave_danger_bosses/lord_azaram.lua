local config = {
    ...
    bossPosition = Position(829, 452, 9), 
    bossArea = {
        fromPos = Position(818, 441, 9), 
        toPos = Position(840, 466, 9), 
        entrancePos = Position(829, 461, 9), 
        exitPosition = Position(828, 478, 9) 
    },
    ...
    createTeleportPos = Position(821, 453, 9), 
    teleportToPosition = Position(828, 478, 9), 
...
local lordAzAction = Action()

function lordAzAction.onUse(player, item, fromPos, target, toPos, isHotkey)
    local participants = {}
    ...
    boss:registerEvent("lordAzSystemDeath")
...
lordAzAction:aid(config.actionId)
lordAzAction:register()

local creatureEvent = CreatureEvent("lordAzSystemDeath")
