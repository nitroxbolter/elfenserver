local config = {
    actionId = 27102, 
    bossName = "ghulosh",
    bossPosition = Position(918, 1591, 9), 
    bossArea = {
        fromPos = Position(906, 1583, 9), 
        toPos = Position(931, 1605, 9), 
        entrancePos = Position(918, 1600, 9), 
        exitPosition = Position(916, 1673, 9) 
    },
    ...
    createTeleportPos = Position(918, 1586, 9), 
    teleportToPosition = Position(916, 1673, 9), 
...
local guloshAction = Action()

function guloshAction.onUse(player, item, fromPos, target, toPos, isHotkey)
    local participants = {}
    ...
    boss:registerEvent("guloshSystemDeath")
...
guloshAction:aid(config.actionId)
guloshAction:register()

local creatureEvent = CreatureEvent("guloshSystemDeath")
