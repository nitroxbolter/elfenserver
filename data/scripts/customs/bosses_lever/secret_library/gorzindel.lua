local config = {
    actionId = 27101, 
    bossName = "gorzindel",
    bossPosition = Position(924, 1624, 9), 
    bossArea = {
        fromPos = Position(915, 1615, 9), 
        toPos = Position(934, 1637, 9), 
        entrancePos = Position(923, 1633, 9), 
        exitPosition = Position(916, 1649, 9) 
    },
    ...
    createTeleportPos = Position(923, 1618, 9), 
    teleportToPosition = Position(916, 1649, 9), 
...
local gorzindelAction = Action()

function gorzindelAction.onUse(player, item, fromPos, target, toPos, isHotkey)
    local participants = {}
    ...
    boss:registerEvent("gorzindelSystemDeath")
...
gorzindelAction:aid(config.actionId)
gorzindelAction:register()

local creatureEvent = CreatureEvent("gorzindelSystemDeath")
