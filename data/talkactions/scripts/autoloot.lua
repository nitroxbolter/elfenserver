function onSay(player, words, param)
    ...
    if action == "add" then
        ...
        if not itemType then
            player:sendTextMessage(MESSAGE_STATUS_CONSOLE_BLUE, "Invalid item name or ID.")
            return false
        end
    ...
    elseif action == "remove" then
        local item = split[2]:gsub("%s+", "", 1)
        local success, itemType = pcall(ItemType, item)

        if not success or not itemType then
            player:sendTextMessage(MESSAGE_STATUS_CONSOLE_BLUE, "Invalid item name or ID.")
            return false
        end
