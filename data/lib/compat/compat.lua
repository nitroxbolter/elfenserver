...
function PlayerSetNameDescription() debugPrint("Deprecated function, use Player:onLook event instead.") return true end
function PlayerSendChannelMessage(cid, author, message, SpeakClasses, channel) local p = Player(cid) return p and p:sendChannelMessage(author, message, SpeakClasses, channel) or false end
function PlayerSetMaxCapacity(cid, cap) local p = Player(cid) return p and p:setCapacity(cap) or false end
function PlayerSetSpecialDescription() debugPrint("Deprecated function, use Player:onLook event instead.") return true end
function PlayerSetBalance(cid, balance) local p = Player(cid) return p and p:setBankBalance(balance) or false end
function PlayerSetPromotionLevel(cid, level) local p = Player(cid) return p and p:setVocation(p:getVocation():getPromotion()) or false end
...
function PlayerTakeItem(cid, itemid, count) local p = Player(cid) return p and p:removeItem(itemid, count) or false end
function PlayerTransferMoneyTo(cid, target, money)
...
function PlayerSave(cid) local p = Player(cid) return p and p:save() or false end
...
function PlayerSetStamina(cid, minutes) local p = Player(cid) return p and p:setStamina(minutes) or false end
...
function PlayerSendOutfitWindow(cid) local p = Player(cid) return p and p:sendOutfitWindow() or false end
...
function PlayerUnlearnInstantSpell(cid, name) local p = Player(cid) return p and p:forgetSpell(name) or false end
...
function PlayerAccountManager() debugPrint("Deprecated function.") return true end
function PlayerSetExperienceRate() debugPrint("Deprecated function, use Player:onGainExperience event instead.") return true end
function PlayerSetSkillLevel(cid, skillId, value, ...) local p = Player(cid) return p and p:addSkill(skillId, value, ...) end
function PlayerSetMagicLevel(cid, value) local p = Player(cid) return p and p:addMagicLevel(value) end
function PlayerAddLevel(cid, amount, round) local p = Player(cid) return p and p:addLevel(amount, round) end
...
PlayerAddExperience = PlayerAddExp
