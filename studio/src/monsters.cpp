	...
	if (isScripted) {
		...
		if (tmpName == "melee") {
			...
			if ((attr = node.attribute("fire"))) {
				...
				maxDamage = minDamage;
				tickInterval = 4000;
			} else if ((attr = node.attribute("splash"))) {
				conditionType = CONDITION_SPLASHED;

				minDamage = pugi::cast<int32_t>(attr.value());
				maxDamage = minDamage;
				tickInterval = 4000;
			} else if ((attr = node.attribute("bewitched"))) {
				conditionType = CONDITION_BEWITCHED;
			...
			if (tmpName == "firecondition") {
				...
				conditionType = CONDITION_CURSED;
				tickInterval = 4000;
			} else if (tmpName == "splashcondition" || tmpName == "splashcondition") {
				conditionType = CONDITION_SPLASHED;
				tickInterval = 4000;
			} else if (tmpName == "bewitchedcondition" || tmpName == "bewitchedcondition") {
				conditionType = CONDITION_BEWITCHED;
				tickInterval = 4000;
		...
		for (auto immunityNode : node.children()) {
			if ((attr = immunityNode.attribute("name"))) {
				...
				if (tmpStrValue == "physical") {
					...
					mType->info.conditionImmunities |= CONDITION_SPLASHED;
				} else if (tmpStrValue == "arcane") {
					mType->info.damageImmunities |= COMBAT_ARCANEDAMAGE;
					mType->info.conditionImmunities |= CONDITION_BEWITCHED;
