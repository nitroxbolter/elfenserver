...
MoveEvent* MoveEvents::getEvent(Item* item, MoveEvent_t eventType, slots_t slot)
{
	...
	switch (slot) {
		...
		case CONST_SLOT_DECK: slotp = SLOTP_DECKBAD; break;
		case CONST_SLOT_BADGE: slotp = SLOTP_DECKBAD; break;
		case CONST_SLOT_SHIP: slotp = SLOTP_SHIP; break;
		case CONST_SLOT_BELT: slotp = SLOTP_BELTGLOVES; break;
		case CONST_SLOT_GLOVES: slotp = SLOTP_BELTGLOVES; break;
	...
	if (eventType == MOVE_EVENT_EQUIP || eventType == MOVE_EVENT_DEEQUIP) {
		...
		if (slotAttribute) {
			...
			if (tmpStr == "head") {
				...
				slot = SLOTP_AMMO;
			} else if (tmpStr == "deckbadge") {
                slot = SLOTP_DECKBAD;
            } else if (tmpStr == "ship") {
                slot = SLOTP_SHIP;
            } else if (tmpStr == "beltgloves") {
                slot = SLOTP_BELTGLOVES;
            } else {
